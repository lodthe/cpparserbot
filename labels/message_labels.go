...
`
	GetBinancePricesList = `Выберите интересующую Вас пару из списка или отправьте сообщение в таком же формате.
`
	GetAllPrices = `Документ с последней информацией о всех курсах (Откуда парсить??).
`
	UnknownCommand = `Неизвестная команда :( Попробуйте ещё раз.
` + CommandList
